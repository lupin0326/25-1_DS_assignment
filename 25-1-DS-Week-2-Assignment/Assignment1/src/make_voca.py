...
def merge_vocab(pair, vocab):
    ...
    for token_seq in vocab.keys():
        ...
        merged_vocab[new_token_seq] = vocab[token_seq]
    return merged_vocab
...
def compute_likelihood_score(pair, pair_freq, unigram_counts, total_tokens):
    ...
    token1, token2 = pair
    freq_token1 = unigram_counts[token1]
    freq_token2 = unigram_counts[token2]
    expected_freq = (freq_token1 * freq_token2) / total_tokens
    score = pair_freq * math.log(pair_freq / expected_freq)
    return score
